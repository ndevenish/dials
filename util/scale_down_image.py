...
def scale_down_array_py(image, scale_factor):
    ...
    Input data type must be integers; negative values assumed to be
    flags of some kind (i.e. similar to Pilatus data) and hence
    preserved as input.
    """
...
def write_image_from_flex_array(out_image, pixel_values, header):
    """
    Write a scaled CBF image from an array of pixel values and a header to add
    at the top.

    N.B. clobbers the binary size of the compressed data & the MD5 hash
    of the data.
    """
...
def scale_down_image(in_image, out_image, scale_factor):
    """Read in the data from in_image, apply the statistically valid scale
    factor to the data & write this out as out_image; retain the header as we
    go."""
