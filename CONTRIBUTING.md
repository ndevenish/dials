...
- Include a newsfragment before merging. This is a file with a description of
  your change intended for users of DIALS, to go in the CHANGELOG and release
  notes. As such it should often be less technical than the commit message.
  This is a file in the `newsfragments` subdirectory named
  `<number>.<category>` - where `<number>` is either the issue or pull request
  number, and category is the type of chane being made - our current list of categories can be found [here][newsREADME]. These are processed on release
  by a tool named [towncrier].
...
- **We format python code with [black]**. This means that while writing code
  ...
  some slips through, the whole codebase is auto-cleaned once a week. Most IDEs and
  editors have support for running formatters like black frequently or
  automatically.
- **We sort python imports with [isort]**. This means that imports will be
  moved to an appropriate section, and ordered by name. By default, this won't
  reorder imports and code, so you shouldn't need to worry about it breaking
  anything - python modules should be written to be import-order independent.
  In the rare case where you need explicit control, you can turn this off
  selectively - see the isort documentation.
